...
class ComplexResidualConvBlock(nn.Module):
    def __init__(self, c):
        super().__init__()
        self.norm1 = ComplexLayerNorm(c)
        self.conv1 = ComplexConv2dReflect(c, c, kernel_size=3, padding=1)
        self.act1 = ComplexGELU()
        self.norm2 = ComplexLayerNorm(c)
        self.conv2 = ComplexConv2dReflect(c, c, kernel_size=3, padding=1)
    ...
    def forward(self, x):
        ...
        out = self.conv2(out)
        out = self.act2(out)
        return out * self.gamma.type_as(out.real) + x
...
class GuidedComplexConvNet(nn.Module):
    """
    Complex-convolution baseline with the same 2.5D encoder-decoder scaffold as GuidedComplexNAFNet.

    Differences from the NAFNet-style model:
    - replaces ComplexNAFBlock with a standard residual complex conv block
    - removes multiplicative simple-gate and channel attention inside the block
    - keeps reflect padding, anti-aliased downsampling, and bilinear upsampling
    """
    ...
    def __init__(self, dwi_channels=3, out_channels=1, num_slices=3, support_k=0, max_targets=1):
        ...
        self.enc1 = nn.Sequential(ComplexResidualConvBlock(32), ComplexResidualConvBlock(32))
        self.pool1 = ComplexAntiAliasDownsample(32, 64)

        self.enc2 = nn.Sequential(ComplexResidualConvBlock(64), ComplexResidualConvBlock(64))
        self.pool2 = ComplexAntiAliasDownsample(64, 128)

        self.enc3 = nn.Sequential(ComplexResidualConvBlock(128), ComplexResidualConvBlock(128))
        self.pool3 = ComplexAntiAliasDownsample(128, 256)

        self.bottleneck = nn.Sequential(
            ComplexResidualConvBlock(256),
            ComplexResidualConvBlock(256),
            ComplexResidualConvBlock(256),
        )
        ...
        self.dec3 = nn.Sequential(ComplexResidualConvBlock(128), ComplexResidualConvBlock(128))

        self.up2 = ComplexUpsample(128, 64, blur_after_up=True)
        self.reduce2 = ComplexConv2dReflect(128, 64, kernel_size=1)
        self.dec2 = nn.Sequential(ComplexResidualConvBlock(64), ComplexResidualConvBlock(64))

        self.up1 = ComplexUpsample(64, 32, blur_after_up=True)
        self.reduce1 = ComplexConv2dReflect(64, 32, kernel_size=1)
        self.dec1 = nn.Sequential(ComplexResidualConvBlock(32), ComplexResidualConvBlock(32))
